package webview.rea.com.webview.x5.tools;
...
import webview.rea.com.webview.R;
...
public class CommonUtils {
    ...
    public static Intent getInstallIntent(File apkFile) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            String auth = WebViewLibrary.get().getResources().getString(R.string.AutoUpdateprovide);
            uri = FileProvider.getUriForFile(WebViewLibrary.get(), auth, apkFile);
...
//        return AndroidProcesses.isMyProcessInTheForeground();
    ...
    public static Class<Activity> getLaunchActivity() {
        try {
            PackageInfo packageInfo = WebViewLibrary.get().getPackageManager().getPackageInfo(WebViewLibrary.get().getPackageName(), 0);
            ...
            List<ResolveInfo> list = WebViewLibrary.get().getPackageManager().queryIntentActivities(resovleIntent, 0);
